...
namespace na
{
	namespace util
	{
		...
		void GLog::openOrCreateFile( string fileName )
		{
			...
			string fanalLogFileName;
			int indexFile = 0;
			do 
			{
				fanalLogFileName = lastLogFullFileName + "-" + boost::lexical_cast<string, int>(indexFile) + ".log";
				if(! boost::filesystem::exists(fanalLogFileName) || indexFile > 1000){
					logOfstream.open(fanalLogFileName.c_str(),ios_base::app);
					break;
				}
				++indexFile;
			} while (true);
...
//			logOfstream.open(lastLogFullFileName.c_str(),ios_base::app);
			...
			cout << "GLog open file:" << fanalLogFileName << endl;
		}
		...
		GLog& GLog::preLog(GLogLevel level,GLogColor color,const char* functionName,int lineNumber)
		{
			...
			if(isOutPutFile)
			{
				...
				{
					openOrCreateFile(this->serverName+"-"+getCurrentDateTime());
				}
