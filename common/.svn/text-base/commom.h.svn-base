...
public:
	...
	size_t SimpleCnEg(const string& str);
	size_t UTF8Len(const string& str);
	int	random();
	void random_array(int a[], const unsigned length);
	int randomBetween(int low, int high);
	bool randomOk(int rate);//Íò·Ö±È
	bool randomOk(double rate);
	int randomList(const Json::Value &rateList);
	std::string	unshell(const std::string &str);
	int serverID();
