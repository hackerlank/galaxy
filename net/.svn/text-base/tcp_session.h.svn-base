...
namespace na
{
	namespace net
	{
		...
		public:
			...
			inline	int	 get_net_id() const 
			{
				if(NetPtr == NULL)return -1;
				return NetPtr->getID();
			}
			inline int get_vN_net_id() const
			{
				if(NetPtr == NULL)return -1;
				return NetPtr->getVtID();
			}
			inline	void set_net_id(NetInsPtr ptr){NetPtr = ptr;}
			inline	void set_session_status(int s) { _game_status = s;}
			inline	int get_session_status() const {return _game_status;}
			void start(net_handler_pointer handler_ptr);
			void post_read0();
			void post_write0();
			static ptr create(size_t n_buff_size,bool is_tx = false);
			static void destroy(tcp_session* p);
			bool is_alive(const boost::system_time& now_t, const unsigned ms = 180000);
			void keep_alive();
			void stop();
